...
func TestSendProductWebhookEmitsOneNormalizedAnalyticsEvent(t *testing.T) {
	...
	SetMessageSender(func(value any) error {
		// Task telemetry flushes on a timer and may share this sender; only keep analytics events.
		if message := value.(statusMessage); message.Type == "analytics-event" {
			sent = message
		}
		return nil
	})
	...
	if event.Account != "buyer@example.com" || event.Profile != "Main Profile" {
		t.Fatalf("account fields = %#v", event)
	}
...
func TestSendCartedAnalyticsUsesProvidedGrandTotal(t *testing.T) {
	...
	SendCartedAnalytics(ProductWebhookData{
		Site: "Target", Email: "target@example.com", ProfileName: "Target Main",
		TaskID: "run-2", ClientTaskID: "task-2", RunID: "run-2", GrandTotal: 19.99,
	...
	if messages[0].Account != "target@example.com" || messages[0].Profile != "Target Main" {
		t.Fatalf("account fields = %#v", messages[0])
	}
}
