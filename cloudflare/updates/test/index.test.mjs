...
test('routes each Mac download to its own architecture feed', async () => {
  ...
  assert.equal(arm.headers.get('location'), 'https://updates.rcart.app/mac/arm64/Zyn-1.6.81-arm64.dmg');
  assert.equal(intel.headers.get('location'), 'https://updates.rcart.app/mac/x64/Zyn-1.6.81-x64.dmg');
});
...
test('keeps redirects on whichever Zyn domain served the request', async () => {
  ...
  assert.equal(response.headers.get('location'), 'https://updates.zynbot.app/mac/arm64/Zyn-1.6.81-arm64.dmg');
});
...
test('serves architecture metadata and rejects unknown architectures', async () => {
  const env = { RELEASES: releaseStore({
    'mac/x64/latest-mac.yml': 'version: 1.6.81\n',
  }) };
...
test('returns a no-artifact current-version feed before a signed release is published', async () => {
  ...
  for (const arch of ['arm64', 'x64']) {
    ...
    assert.match(await response.text(), /^version: 1\.6\.81\nfiles: \[\]/);
  }
});
