...
const service = 'com.thwebco.zyn.checkout-webhook';

async function readWebhook() {
  const configured = String(process.env.ZYN_GLOBAL_CHECKOUT_WEBHOOK || '').trim();
  if (configured) return configured;

  process.stderr.write('Paste the global Discord checkout webhook, then press Return:\n');
...
async function main() {
  ...
  let url;
  try { url = new URL(webhook); } catch { throw new Error('The supplied checkout webhook is not a URL.'); }
  ...
  if (url.protocol !== 'https:'
    ...
    throw new Error('The supplied checkout webhook is not a valid Discord webhook URL.');
  }

  execFileSync('security', [
    'add-generic-password', '-U', '-a', account, '-s', service, '-w', webhook,
  ], { stdio: ['ignore', 'ignore', 'inherit'] });
  console.log('Global checkout webhook saved to Keychain for release builds.');
}
