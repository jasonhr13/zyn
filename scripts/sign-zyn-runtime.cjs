...
// Kept separate from app signing so downloadable Chromium can be independently notarized and
// verified after extraction. This is the Zyn-specific runtime signer.
...
const entitlements = path.resolve(process.argv[3] || path.join(__dirname, '..', 'release', 'entitlements.mac.plist'));
...
if (!bundle || !fs.existsSync(bundle)) {
  console.error('Usage: node scripts/sign-zyn-runtime.cjs <bundle.app> [entitlements.plist]');
  process.exit(1);
}
if (!fs.existsSync(entitlements)) throw new Error(`Entitlements file not found: ${entitlements}`);
...
const candidates = found.files.map((file) => {
  let description = '';
...
console.log(`Signing ${candidates.length} runtime Mach-O files in ${bundle}`);
for (const [index, item] of candidates.entries()) {
  const args = ['--force', '--timestamp', '--options', 'runtime', '--sign', identity];
  if (/executable/.test(item.description)) args.push('--entitlements', entitlements);
  args.push(item.file);
...
for (const nested of found.bundles.sort((a, b) => b.split(path.sep).length - a.split(path.sep).length)) {
  const args = ['--force', '--timestamp', '--options', 'runtime', '--sign', identity];
  if (/\.(app|xpc)$/i.test(nested)) args.push('--entitlements', entitlements);
  args.push(nested);
  execFileSync('/usr/bin/codesign', args, { stdio: 'inherit' });
}
...
console.log(`Signed runtime bundle: ${bundle}`);
