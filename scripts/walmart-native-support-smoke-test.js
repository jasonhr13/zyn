...
assert.match(page, /Raffle Entry/);
assert.match(page, /WALMART_MODES/);
assert.match(engine, /normalizeWalmartMode/);
assert.match(engine, /mode: normalizeWalmartMode\(task\.mode \|\| shared\.mode\)/);
assert.match(walmartCheckout, /wait-for-draw/);
assert.match(walmartCheckout, /draw-preferences/);
assert.match(walmartCheckout, /draw-enter/);
const walmartInit = fs.readFileSync(path.join(engineSourceRoot(), 'sites/walmart/init.go'), 'utf8');
const walmartRequest = fs.readFileSync(path.join(engineSourceRoot(), 'sites/walmart/request.go'), 'utf8');
assert.match(walmartInit, /isRaffleMode/);
assert.match(walmartFunctions, /draw\.www\.walmart\.com/);
assert.match(walmartRequest, /\/draw\/preferences/);
assert.match(walmartRequest, /\/draw\/enter/);
assert.match(walmartFunctions, /AddCookiesToDraw/);
...
assert.match(accounts, /<option value="walmart">Walmart<\/option>/);
...
assert.match(page, /connectEngineLog\('walmart'\)/);
assert.match(page, /pickTableState\(state\.walmart/);
assert.doesNotMatch(page, /engineLogRef/);
