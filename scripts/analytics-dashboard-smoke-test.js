...
assert.match(dashboard, /Search account, product, order/);
...
assert.match(worker, /account: analyticsText\(value\.account, 254\)/);
assert.match(worker, /e\.account LIKE \? COLLATE NOCASE/);
assert.match(migration, /PRIMARY KEY \(user_id, event_id\)/);
const accountMigration = read('cloudflare/license/migrations/0014_analytics_account.sql');
assert.match(accountMigration, /ADD COLUMN account TEXT NOT NULL DEFAULT ''/);
assert.match(accountMigration, /ADD COLUMN profile TEXT NOT NULL DEFAULT ''/);
