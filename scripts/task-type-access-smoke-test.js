...
assert.deepEqual(normalizeTaskTypeAccess(), { pokemoncenter: false, round1: false });
assert.deepEqual(normalizeTaskTypeAccess({ pokemoncenter: true, round1: 'true', future: true }), {
  pokemoncenter: true,
  round1: false,
});
assert.deepEqual(normalizeTaskTypeAccess({}, true), { pokemoncenter: true, round1: true });
...
assert.deepEqual(blocked.map(event => event.taskType), ['round1', 'pokemoncenter']);

status = { ok: true, taskTypes: { pokemoncenter: true, round1: true } };
const allowedRound1 = {};
...
status = { ok: false, taskTypes: { pokemoncenter: false, round1: false } };
