...
assert.match(settings, /saved site[\s\S]{0,40}login cookies/);
assert.match(settings, /Zyn account\/session credentials[\s\S]{0,120}managed-proxy service credentials are excluded/);
assert.doesNotMatch(settings, /browser session cookies/);
